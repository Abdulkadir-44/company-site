...
function ServiceCard({ service, index }: { service: typeof servicesData[0]; index: number }) {
    ...
    const mobileImageUrl = service.mobileImageUrl || service.imageUrl;
    ...
    return (
        <motion.div
            ...
            className="group"
        >
            <div className={`relative bg-white rounded-3xl py-8 px-4 lg:p-10 shadow-lg hover:shadow-2xl transition-all duration-500 border ${service.borderColor} hover:border-transparent overflow-hidden`}>

                {/* Mobile Background Image - Sadece mobilde görünür */}
                <div className="absolute inset-0 lg:hidden">
                    <div
                        className="absolute bottom-0  right-0 w-3/4 h-2/3 opacity-90 bg-cover bg-no-repeat rounded-tr-3xl"
                        style={{
                            backgroundImage: `url(${mobileImageUrl})`,
                            maskImage: 'linear-gradient(135deg, transparent 0%, rgba(0,0,0,0.6) 30%, rgba(0,0,0,0.8) 60%, transparent 100%)',
                            WebkitMaskImage: 'linear-gradient(135deg, transparent 0%, rgba(0,0,0,0.6) 30%, rgba(0,0,0,0.8) 60%, transparent 100%)'
                        }}
                    />
                    {/* Yazıları koruyan overlay */}
                    <div className="absolute inset-0 bg-white/50 backdrop-blur-[0.5px]" />
                </div>

                {/* Desktop Background Pattern - Mevcut */}
                <div className="absolute top-0 right-0 w-32 h-32 opacity-5 hidden lg:block">
                    <div className={`w-full h-full bg-gradient-to-br ${service.gradient} rounded-full blur-3xl`}></div>
                </div>

                <div className={`relative z-10 flex flex-col lg:flex-row items-stretch gap-8 ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {service.tags.map((tag, tagIndex) => (
                                <span
                                    key={tagIndex}
                                    className={`px-4 py-2 ${service.bgColor} border ${service.borderColor} rounded-full text-sm font-medium text-gray-700 hover:shadow-md transition-all duration-300 backdrop-blur-sm`}
                                >
                                    {tag}
                                </span>
                            ))}
                    ...
                    {/* Image Side - Sadece desktop'ta görünür */}
